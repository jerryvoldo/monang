      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <div class="card-body">
              <table id="table_realisasi_tup" class="table table-bordered table-striped">
                    <thead>
                    <tr>
                    ...
                    <tbody>
                      <?php foreach($load_all_realisasi_tup as $tup):?>
                        <tr>
                          ...
                          <td><?=strtoupper($tup['cara_tarik'])?></td>
                          <td class="text-center">
                              <?php if($tup['sumber'] == 'rm'):?>
                                <h4><span class="badge badge-pill badge-primary">RM</span></h4>
                              <?php else:?>
                                <h4><span class="badge badge-pill badge-danger">PNP</span></h4>
                              <?php endif;?>
                            </td>
                          <td class="text-center"><?= ($tup['is_kontraktual'] ? '<span class="badge badge-pill badge-info">KONTRAK</span>' : '<span class="badge badge-pill badge-light">BUKAN</span>') ?></td>
                          <td>
                            <span class="font-weight-bold"><?=$tup['nomor_spm']?></span><br>
                             <?=date('d M Y', $tup['epoch_spm'])?>
                          </td>
                        </tr>
                      <?php endforeach;?>
                    </tbody>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <!-- SELECT2 EXAMPLE -->
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-8">
                        ...
                        <table class="table table-sm table-bordered">
                          ...
                          <tr>
                            <th>Sisa Pagu</th>
                            <td class="text-right" id="sisa_pagu" colspan="6">0</td>
                          </tr>
                        </table>
                      </div>
                ...
                <div class="card card-default shadow ">
                  <div class="card-header">
                    <h4 class="card-title">Input Data Realisasi TUP</h4>
                  </div>
                  <form method="POST" action="<?=base_url('index.php/dashboard/simpan_realisasi')?>">
                    ...
                    <input type="hidden" name="cara_tarik" value="tup">
                  <div class="card-body">
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                        ...
                        <div class="row">
                          <div class="col-md-4">
                            <div class="form-group">
                              ...
                              <input class="form-control" type="number"  name="jumlah_realisasi_bruto"  id="jumlah_realisasi_bruto" onkeyup="hitung_nilai_ppn()">
                            </div>
                          </div>
                          <div class="col-md-4">
                            <div class="form-group">
                              <input type="checkbox" id="checked_ppn" onclick="hitung_nilai_ppn()">
                              <label>PPn 10%</label>
                              <input class="form-control" type="number"  name="nilai_ppn" id="nilai_ppn" readonly>
                            </div>
                          </div>
                          <div class="col-md-4">
                            <div class="form-group">
                              <input type="checkbox" id="checked_pph" onclick="hitung_nilai_pph()">
                              <label>PPh</label>
                              <input class="form-control" type="number"  name="nilai_pph" id="nilai_pph" readonly>
                              <select class="form-control" name="select_pph" id="select_pph" onchange="hitung_nilai_pph()">
                                <option value="0" selected>Tanpa PPh</option>
                                <option value="0">PPh 21 0% (PNS Gol I/II)</option>
                                <option value="0.05">PPh 21 5% (PNS Gol III)</option>
                                <option value="0.15">PPh 21 15% (PNS Gol IV)</option>
                                <option value="0.025">PPh 21 5% x 50% (non pegawai, NPWP)</option>
                                <option value="0.03">PPh 21 5% x 50% x 120% (non pegawai, non NPWP)</option>
                                <option value="0.015">PPh 22 1,5%</option>
                                <option value="0.02">PPh 23 2% (sewa, imbalan, jasa teknik)</option>
                                <option value="no_pph">PPh 23 15% (dividen, bunga, royalti)</option>
                              </select>
                            </div>
                          </div>
                        </div>
            ...
            <!-- script untuk hitung pajak -->
            <script type="text/javascript">
              // get all form pajak
              let jumlah_realisasi_bruto = document.getElementById('jumlah_realisasi_bruto');
              let checked_ppn = document.getElementById('checked_ppn');
              let checked_pph = document.getElementById('checked_pph');
              let nilai_ppn = document.getElementById('nilai_ppn');
              let nilai_pph = document.getElementById('nilai_pph');
              let select_pph = document.getElementById('select_pph');

              function hitung_nilai_ppn()
              {
                if(checked_ppn.checked)
                {
                  nilai_ppn.value = Math.ceil(0.1 * parseInt(jumlah_realisasi_bruto.value));
                  hitung_nilai_pph();
                }
                else
                {
              ...
              function hitung_nilai_pph()
              {
                if(checked_pph.checked)
                {
                  let persen_pph = select_pph[select_pph.selectedIndex];
                  if(checked_ppn.checked)
                  {
                    nilai_pph.value = Math.ceil(parseFloat(persen_pph.value) * (parseInt(jumlah_realisasi_bruto.value) - Math.ceil(0.1 * parseInt(jumlah_realisasi_bruto.value))));

                  }
                  else
                  {
                    nilai_pph.value = Math.ceil(parseFloat(persen_pph.value) * parseInt(jumlah_realisasi_bruto.value));
                  }
                }
                else
                {
